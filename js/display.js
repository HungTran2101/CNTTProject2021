...
//bật tắt nhạc của game
function toggleMusic() {
  if (!isMusic) {
    music.play();
    isMusic = true;
    document.getElementById("soundBtn").style.backgroundImage =
...
//Kiểm tra dữ liệu input của người dùng
...
//thực hiện gửi request signin tới server
function login() {
  let login = document.getElementById("loginBtn");
...
//gán dữ liệu người dùng vào các biến để xử lí khi chơi
...
//cấu hình cookies sau khi người dùng login thành công
function setCookies() {
  document.cookie = `id=${userID}; expires=Thu, 18 Dec 2022 12:00:00 UTC`;
  document.cookie = `highscore=${Math.round(
    highscore
  ...
  document.cookie = `wolf=${wolfSkin
    .toString()
    .replace(/,/g, "")}; expires=Thu, 18 Dec 2022 12:00:00 UTC`;
  document.cookie = `bear=${bearSkin
    .toString()
    .replace(/,/g, "")}; expires=Thu, 18 Dec 2022 12:00:00 UTC`;
  document.cookie = `cactus=${cactusSkin
    .toString()
    .replace(/,/g, "")}; expires=Thu, 18 Dec 2022 12:00:00 UTC`;
}

//hiện giao diện sau khi người dùng login thành công
...
//xóa cookies của người dùng khi logout
function logout() {
...
//xử lí hiển thị trang phục theo tham số option khi người dùng lựa chọn loại trang phục
...
//cập nhật trạng thái của các nút mua trang phục
...
//cập nhật skin mặc định cho nhân vật, vật cản
